...
//				        title: myData[i].hotelNo
	    	...
	    	// 클릭시 리스트 이동 함수 
	       	 function makeClickListener(marker, i, myData) {
	       	     return function() {
	       	    	 ...
	       	    	 var myScroll = $(document.getElementById(myData[i].hotelNo)).offset().top;
	       ...
	       // 호텔 리스트 테이블을 생성
	        //$('#closeHotelList').append('<div></div>');
	       	var div = $('#closeHotelList'); //.children();
	       	...
	       	for(var i=0; i<4; i++) {
	       		...
	       		div.append(
   					  ...
   					  +	'<div class="info-box">'
   					  +	'<div><span>' + hotelArray[i].grade + '</span></div>'
					  +	'<div class="hotelIndex" id="' + hotelArray[i].hotelNo + '">'
					  ...
					  +	'</div>'
					  +	'<div class="distance">'
					  +	'<span class="num">' + (hotelArray[i].loadLength < 1000 ? hotelArray[i].loadLength : Number(hotelArray[i].loadLength/1000).toFixed(2)) + '</span>'
					  + '<span id="km">km</span>'
					  +	'</div>'
					  +	'<div class="star"><span>' + star + '</span></div>'
					  +	'<div><span>' + hotelArray[i].address + '</span></div>'
					  +	'<div><span>' + hotelArray[i].descript + '</span></div>'
					  +	'</div></div>'
   					  );
	       	}
	       	$("#closeHotelList").bxSlider({
	    		auto: true, 
	    		mode: 'fade',
	    		controls: false,
	    		autoHover: true
