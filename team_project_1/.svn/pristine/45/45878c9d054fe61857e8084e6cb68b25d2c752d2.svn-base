...
(function (root, factory) {
    ...
    Lightpick.prototype = {
        ...
        hide: function()
        {
            ...
            // renewURL += '&page='+page;
            renewURL += '?res_Start=' +  $("#res_Start").val() + '&res_End=' + $("#res_End").val() + '&hotelNo=' + $("#hotelNo").val();
            //페이지 갱신 실행!
            ...
            $.ajax("roomsPage", {
            	dataType: "json",
            	async: true,
            	data: "res_Start=" + $("#res_Start").val() + "&hotelNo=" + $("#hotelNo").val(),
            	success : function(data) {
                    $("#content-reserve").empty();
                    $("#content-reserve").append("<ul></ul>");
                    var div = '';

                    for(var i=0; i<data.length; i++){
                        if(data[i].maxRoom <= data[i].count){
                            div += '<li>'
                                        +'<div class="box">'
                                        +'<div class="pic">'
                                        +'<img src="images/hotel3.jpg"'
                                        +' style="width: 350px; hegiht: 220px;">'
                                        +'</div>'
                                        +'<div class="info">'
                                        +'<div class="half-right">'
                                        +'<strong class="room-title">'+ data[i].roomName +'</strong>'
                                        +'<span class="room-content">숙박</span>'
                                        +'<div class="infor-time">'
                                        +'<div class="price">'
                                        +'<span class="price-text">숙박요금</span>'
                                        +'<span class="room-price">'+ data[i].price +'<i>원</i></span>'
                                        +'</div>'
                                        +'<hr><ul>'
                                        +'<li><span>입실시간</span> 17시부터</li>'
                                        +'<li><span>퇴실시간</span> 익일 12시</li>'
                                        +'</ul>'
                                        +'</div>'
                                        +'<button type="button" class="reserve_End">예약 마감</button>'
                                        +'</div></div></div></li>';
                        }else{
                            div += '<li>'
                                        +'<div class="box">'
                                        ...
                                        +'</ul>'
                                        +'</div>'
                                        +'<a href="resPage?res_Start='+ $("#res_Start").val() +'&res_End='+ $("#res_End").val() +'&roomNo='+ data[i].roomNo +'"><button '
										+'type="button" class="borrow">숙박 예약</button></a>'
                                        +'</div></div></div></li>';
                        }
                    }

                    $("#content-reserve").children().html(div);
            	}
            })
